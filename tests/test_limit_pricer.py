...
@pytest.mark.parametrize("side,bid,ask,tick,exp",[
...
def test_offset_rounding(side,bid,ask,tick,exp):
    ...
    cfg=LimitsConfig(buy_offset_frac=0.25,sell_offset_frac=0.25,
                     max_offset_bps=1000,wide_spread_bps=100,
                     escalate_action="cross",stale_quote_seconds=10,
                     use_ask_bid_cap=True)
    p,t=calc_limit_price(side,"T",tick,prov,now,cfg)
    assert t=="LMT" and p==pytest.approx(exp)

@pytest.mark.parametrize("side,bid,ask,maxbps,exp",[
...
def test_nbbo_maxoffset(side,bid,ask,maxbps,exp):
    ...
    cfg=LimitsConfig(buy_offset_frac=1.0,sell_offset_frac=1.0,
                     max_offset_bps=maxbps,wide_spread_bps=100,
                     escalate_action="cross",stale_quote_seconds=10,
                     use_ask_bid_cap=True)
    p,t=calc_limit_price(side,"T",0.01,prov,now,cfg)
    assert t=="LMT" and p==pytest.approx(exp)

@pytest.mark.parametrize("bid,ask,delta,action,exp,t",[
...
def test_wide_or_stale_escalation(bid,ask,delta,action,exp,t):
    ...
    cfg=LimitsConfig(buy_offset_frac=0.25,sell_offset_frac=0.25,
                     max_offset_bps=10,wide_spread_bps=50,
                     escalate_action=action,stale_quote_seconds=10,
                     use_ask_bid_cap=True)
